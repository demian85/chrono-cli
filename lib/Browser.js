...
class Browser {
  ...
  async parseData() {
    ...
    const columns = await page.$$eval(`.persist-area thead th.day`, async (elements) => {
    ...
    const dayColumnIndex = columns.dates.findIndex(date => date === currentDate);

    if (dayColumnIndex === -1) {
      throw new Error('column for today does not exist!');
    }
    ...
    const summary = [
      [chalk.bold.underline('Project'), ...columns.names, chalk.bold.underline('TOTAL')],
      ...projects.map(project => [project.name, ...project.weekHours]),
      weekTotals
    ];
